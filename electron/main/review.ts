...
export async function reviewPullRequest(prId: number): Promise<ReviewResultPayload> {
  ...
  try {
    ...
    const droppedInline = findings.filter((finding) => !finding.location).length;
    if (droppedInline) {
      appendLog(
        "warn",
        `PR #${prId}: dropped ${droppedInline} inline finding(s) whose file/line was not in the annotated diff`,
      );
    }
...
${
  findings.length
    ? findings
        .map((f) => {
          const loc = f.location ? `${f.location.file}:${f.location.line}` : `${f.file}:${f.line} (not in diff; skipped inline)`;
...
`;
    ...
    if (auth.writeMode === "bitbucket") {
      const skippedNote =
        droppedInline > 0
          ? `\n\n_${droppedInline} finding(s) were not posted inline because the line was not in the new-file diff._`
          : "";
      const commentBody = `## AI Code Review\n\n${parsed.summary}${skippedNote}\n\n---\n_Generated by Code Review Agent_`;
