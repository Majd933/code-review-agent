...
import { annotateDiff } from "./diff-annotate";
...
function formatFindingBullet(finding: {
  ...
  const where =
    finding.file && Number.isFinite(finding.line) && finding.line > 0
...
function buildGeneralCommentBody(summary: string, findings: Array<{
  severity: string;
  file: string;
  line: number;
  message: string;
  ...
  const findingsBlock = findings.length
    ? `\n\n### Findings\n\n${findings.map(formatFindingBullet).join("\n")}`
    : "";
  return `## AI Code Review\n\n${summary}${findingsBlock}\n\n---\n_Generated by Code Review Agent_`;
}
...
${
  findings.length
    ? findings.map((f) => formatFindingBullet(f)).join("\n")
...
`;
    ...
    if (auth.writeMode === "bitbucket") {
      if (!parsed.parsed) {
        appendLog("warn", `PR #${prId}: JSON parse failed; posting general comment from raw summary`);
      }
      await postGeneralComment(
        { bitbucketUrl: auth.bitbucketUrl, project: auth.project, repository: auth.repository, token: auth.token },
        prId,
        buildGeneralCommentBody(parsed.summary, findings),
      );
      posted = true;
      appendLog("info", `Posted general review comment to Bitbucket PR #${prId}`);
    } else {
